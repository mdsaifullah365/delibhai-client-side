      ...
      <div className='absolute bottom-3 left-1/2 -translate-x-1/2 flex'>
        {Array.from({ length: slides.length }).map((item, index) => (
          <div
            ...
            } w-[1.63733vw] md:w-4 h-[1.63733vw] md:h-4 rounded-full border-[0.3276vw] md:border-2 border-[#ffffff] mx-[1.63866vw] md:mx-3 transition-colors duration-1000 ease-in-out`}></div>
        ))}
      </div>
    </div>
  );
