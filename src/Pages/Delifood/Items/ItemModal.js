...
const ItemModal = ({ item, setItemModal }) => {
  ...
  return (
    <>
      ...
      ></div>
      <div className="max-w-[640px] mx-6 sm:mx-auto h-fit sm:h-[90vh] my-auto bg-white fixed top-0 right-0 bottom-0 left-0 z-[10000] overflow-y-scroll overflow-x-hidden scrollbar scrollbar-thumb-transparent scrollbar-track-transparent scrollbar-thin">
        <div className="sticky top-0 left-0 right-0 w-full h-2 sm:h-4 bg-white z-[10001]">
          <div
            onClick={() => setItemModal(null)}
            className="absolute top-0 right-0 w-4 sm:w-8 h-4 sm:h-8 bg-gray-200 rounded-full z-[100000] translate-[-50%, -50%] flex justify-center items-center cursor-pointer"
          >
            <div>X</div>
          </div>
        </div>
